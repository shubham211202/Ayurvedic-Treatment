...
public class BMICalculatorActivity extends AppCompatActivity
    ...
    TextView txtWeight, txtHeightFeet, txtHeightInches;
    EditText edtWeight, edtHeightFeet, edtHeightInches;
    ...
    private void initView() {
        ...
        txtHeightFeet = findViewById(R.id.txtHeightFeet);
        txtHeightInches = findViewById(R.id.txtHeightInch);

        edtWeight = findViewById(R.id.edtWeight);
        edtHeightFeet = findViewById(R.id.edtHeightFeet);
        edtHeightInches = findViewById(R.id.edtHeightInch);
    ...
    private boolean isDataValid() {
        ...
        if (TextUtils.isEmpty(edtHeightFeet.getText().toString().trim())) {
    ...
    private void onSelectedCalculateBMI() {
        if (isDataValid()) {
            double weight = Double.parseDouble(edtWeight.getText().toString().trim());
            double height = Double.parseDouble(edtHeightFeet.getText().toString().trim()) * 12.0;

            String heightInchStr = edtHeightInches.getText().toString().trim();
            if (!TextUtils.isEmpty(heightInchStr)) {
                height += Double.parseDouble(heightInchStr);
            }
            ...
            double bmiValue = Double.parseDouble(df.format(bmiValueUnformatted));
    ...
    private void showBMIStatusDialog(double value, String status, final int bmiCase) {
        ...
        TextView txtBMIValue = view.findViewById(R.id.txtBMIValue);
        TextView txtBMIStatus = view.findViewById(R.id.txtBMIStatus);
        Button btnCancel = view.findViewById(R.id.btnCancel);
        Button btnSuggestions = view.findViewById(R.id.btnSuggestions);

        txtBMIValue.setText(String.valueOf(value));
    ...
    public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
        if (v.getId() == R.id.edtHeightInch && actionId == EditorInfo.IME_ACTION_DONE) {
            hideKeyboard();
